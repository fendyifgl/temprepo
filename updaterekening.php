<?php
	...
	if ($submitapi) {
		...
		if (empty($norekening) && $listbankcheck) {
			...
			if (@$arr['NOREKENING'] == $norekening) {
				...
				$response = checkapi($prefix, $noper, $kdbank, $norekening, $swifts, $DB, $HTTP_HOST_PG_BRI, $HTTP_HOST_PG_BBCA);
			...
			if (@$arr['CODE'] != '0000' && $listbankcheck) {
				...
				if ($submitmanual) {
					$sql = "SELECT count(*) ada 
							FROM TABEL_100_KLIEN_REKENING_TMP
							WHERE prefixpertanggungan = '$prefix'
								AND nopertanggungan = '$noper'
	...
	// Checking API
	function checkapi($prefix, $noper, $kdbank, $norekening, $swifts, $DB, $HTTP_HOST_PG_BRI, $HTTP_HOST_PG_BBCA) {
		$curl = curl_init();
