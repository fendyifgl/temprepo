<?php
	...
	if ($submitapi) {
		...
		if (empty($norekening) && $listbankcheck) {
			...
			if (@$arr['NOREKENING'] == $norekening) {
				...
				//ini
				if (in_array($kdbank, array('212', '125','BTN','126'))) {
					$tombolmanual = true;
				}
				if ($tombolmanual) { // ini
	...
	if ($submitmanual) {
		if(@$arr['NOREKENING'] == $norekening && @$arr['KDBANK'] == $kdbank){
			@$arr['CODE'] == '0000';
	...
	function checkapi($prefix, $noper, $kdbank, $norekening, $swifts, $DB, $HTTP_HOST_PG_BRI, $HTTP_HOST_PG_BBCA,$atasnama) { //ini
		...
		if ($kdbank == '002') {
			...
			// $data = array(
			// 	'responseDescription'  => 'beneficiary_bank_code tidak valid',
			// 	'responseCode' => '',
			// 	'remark2' => 'c',
			// 	'accountName' => 'CORP-02-013',
			// 	'accountNumber' => array('indonesian'=>'beneficiary_bank_code tidak valid')
			// );
		...
		/*=============== Khusus Bank Lain (Kecuali BCA) ===============*/
			...
			// 	'beneficiary_bank_code' => 'c',
			// 	'error_code' => 'CORP-02-013',
			// 	'error_message' => array('indonesian'=>'beneficiary_bank_code tidak valid')
			// );
			$response = curl_exec($curl);
			$data = json_decode($response, true);
		...
		// curl_close($curl);
		// echo $respcode . '-' . $respdescription . '-' . $kdbank.'<br>';
		// echo 'a-'.(!in_array($respcode, array('','CORP-02-013','0103','CORP-00-003'))).'<br>';
		// echo 'b-'.(!in_array($respdescription, array('Anda Tidak Berhak','Transaksi anda tidak dapat diproses','Unknown Account Type','beneficiary_bank_code tidak valid'))).'<br>';
		// echo 'c-'.(!in_array($kdbank, array('212', '125','BTN','126'))).'<br>';
		// echo 'd-'.((1 > 0) && !(in_array($respcode, array('','CORP-02-013','0103','CORP-00-003')) || in_array($respdescription, array('Anda Tidak Berhak','Transaksi anda tidak dapat diproses','Unknown Account Type','beneficiary_bank_code tidak valid')) || in_array($kdbank, array('212', '125','BTN','126')))).'<br>';

		if (count($data) > 0 && !(in_array($respcode, array('','CORP-02-013','0103','CORP-00-003')) || in_array($respdescription, array('Anda Tidak Berhak','Transaksi anda tidak dapat diproses','Unknown Account Type','beneficiary_bank_code tidak valid')) || in_array($kdbank, array('212', '125','BTN','126')))  ) { //ini
			...
			$sql = "INSERT INTO nadm.tabel_100_klien_rekening_api (batch, prefixpertanggungan, nopertanggungan,
							kdbank, tglrequest, request, response, code, description, remark, accountname, accountnumber, 
							currency, status, tglbatch)
						VALUES (TO_CHAR(sysdate, 'yyyymmddhh24mmss'), '$prefix', '$noper', '$kdbank', sysdate, --ini
							utl_raw.cast_to_raw('$request'), '', '$respcode', 
							'$respdescription', '$respremark', '".str_replace("'", "`", $respaccountname)."', '$respaccountnumber', '$respcurrency', 
							'$respstatus', sysdate)";//ini
			$DB->parse($sql);
			$DB->execute();
			echo 'itu';
