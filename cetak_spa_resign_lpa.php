<?
   ...
   $sqlDetail = "SELECT  A.NOAGEN,
	                     A.NOPKAJAGEN,
	                     A.URAIAN,
						 (select NAMA_TTD FROM TABEL_400_PENANDATANGANAN_PKAJ@nadm WHERE a.kdkantor = kode_kantor and jabatan_agen = '29') NAMA_TTD,
                       ...
                       (SELECT NAMA_AGENCY FROM $DBUser.TABEL_001_AGENCY_KANTOR A WHERE KDKANTOR = (SELECT KDKANTOR FROM $DBUser.TABEL_400_AGEN WHERE NOAGEN = C.NOAGEN) ) AS NAMAAGENCY,
	                     B.TGLPKAJAGEN,
	                     A.TGLPENETAPAN,
	                     (SELECT INITCAP(F.NAMAJABATANAGEN) FROM $DBUser.TABEL_413_JABATAN_AGEN F WHERE A.KDJABATANAGEN = F.KDJABATANAGEN) AS JABATAN_BARU,
	                     ...
	                     WHERE P.NOAGEN = A.NOAGEN AND P.TGLJABATAN < A.TGLJABATAN AND (P.STATUS_APPROVAL = '2' OR P.STATUS_APPROVAL IS NULL))AND O.NOAGEN = A.NOAGEN AND ROWNUM <=1)
	                     ) AS JABATAN_LAMA,
    ...
    <tbody>
        ...
        <tr>
            ...
            <?php 
                ...
                $txt = '';
                $jabatanAgen = '';
                if(in_array(ociresult($detail,"KDJABATANAGEN"), $kodeJabatanAgen)){
                  $txt = 'BAS';
                  if(in_array(ociresult($detail,"KDJABATANAGEN"), array('36', '37', '38'))){
                    $grade = substr(ociresult($detail,"JABATAN_BARU"), 6);
                    $jabatanAgen = 'Bancassurance Area Manager - '.$grade;
                  }else{
                    $grade = substr(ociresult($detail,"JABATAN_BARU"), 5);
                    $jabatanAgen = 'Bancassurance Specialist - '.$grade;
                  }
                }elseif (ociresult($detail,"KDJABATANAGEN") == '31') {
                  $txt = 'WST';
                  $jabatanAgen = 'Worksite Specialist';
                }
            ?>
            <td style="width: 1007px;" colspan="5">
              <p>Perjanjian Keagenan Asuransi Jiwa (PKAJ) nomor  <?=$_GET["nopkaj"];?>/IFGL/<?= $txt;?>/<?=substr($_GET["tglpkaj"],3,2);?><?=substr($_GET["tglpkaj"],-2)?> tanggal <? if(substr($_GET["tglpkaj"],3,1) == 0) {$bln = substr($_GET["tglpkaj"],4,1);}else{$bln = substr($_GET["tglpkaj"],3,2);} echo substr($_GET["tglpkaj"],0,2)." ".ucwords(strtolower($nama_bulan[$bln]))." ".substr($_GET["tglpkaj"],-4);?> dan atas nama <?=ucwords(strtolower(ociresult($detail,"NAMAAGEN")));?>.</p>
            </td>
        </tr>
        <tr>
            <td style="width: 1033px;" colspan="6">
        ...
        <tr>
            ...
            <td style="width: 1007px;" colspan="5">
              <p>Surat saudara/i tertanggal TGL/BLN/TAHUN perihal (Alasan Pengunduran Diri)</p>
            </td>
        </tr>
        <tr>
            <td style="width: 26px;">
            ...
            <td style="width: 1007px;" colspan="5">
                <p>Surat Rekomendasi Alternative Distribution Regional Business Manager (ARM) tanggal <? if(substr($_GET["tglpkaj"],3,1) == 0) {$bln = substr($_GET["tglpkaj"],4,1);}else{$bln = substr($_GET["tglpkaj"],3,2);} echo substr($_GET["tglpkaj"],0,2)." ".ucwords(strtolower($nama_bulan[$bln]))." ".substr($_GET["tglpkaj"],-4);?></p>
            </td>
        </tr>
        <tr>
            <td style="width: 26px;">
            ...
            <td style="width: 1007px;" colspan="5">
                <p>Persetujuan dari <?= ociresult($detail,"JABATAN_RAH") ?> tanggal <? if(substr($_GET["tglpkaj"],3,1) == 0) {$bln = substr($_GET["tglpkaj"],4,1);}else{$bln = substr($_GET["tglpkaj"],3,2);} echo substr($_GET["tglpkaj"],0,2)." ".ucwords(strtolower($nama_bulan[$bln]))." ".substr($_GET["tglpkaj"],-4);?></p>
            </td>
        </tr>
        <tr>
            <td style="width: 1033px;" colspan="6">
        ...
        <tr>
            <td style="width: 1033px;" colspan="6">
                <p>Dengan ini PT Asuransi Jiwa IFG memutuskan untuk mengakhiri Perjanjian Keagenan Asuransi Jiwa (PKAJ) :</p>
            </td>
        </tr>
        <tr>
            <td style="width: 1033px;" colspan="6">
        ...
        <tr>
            ...
            <td style="width: 816px;">
                <p><?=$_GET["nopkaj"];?>/IFGL/<?= $txt ?>/<?=substr($_GET["tglpkaj"],3,2);?><?=substr($_GET["tglpkaj"],-2)?></p>
            </td>
        </tr>
        <tr>
            <td style="width: 36px;" colspan="2">
        ...
        <tr>
            ...
            <td style="width: 816px;">
                <p><?=ucwords(strtolower(ociresult($detail,"KTPAGEN")));?></p>
            </td>
        </tr>
...
<p>Berdasarkan hal tersebut agar segala sarana, prasarana, fasilitas dan perangkat yang dipinjamkan Perusahaan selama berlangsungnya Perjanjian Keagenaan Asuransi Jiwa (PKAJ) wajib dikembalikan kepada Perusahaan melalui Alternative Distribution Regional Business Manager (ARM) paling lambat 1 (satu) hari kerja setelah surat ini disampaikan yang dibuktikan dengan bukti tertulis atau tanda terima dokumen.</p>
<p>Apabila dikemudian hari terdapat kerugian, kerusakan, tuntutan, permintaan, pengeluaran dan bentuk kewajiban lainnya sebagai akibat dari kelalaian, tindak penipuan, ketidakjujuran, kecurangan dan/atau pelanggaran yang dilakukan selama menjadi <?= $jabatanAgen ?>, maka yang bersangkutan dan/atau ahli warisnya wajib untuk mengembalikan segala bentuk kerugian dan/atau hutangnya kepada Perusahaan.</p>
...
<tr>
  <td>
  <p><strong>J</strong><strong>JAKARTA</strong><strong>,&nbsp;</strong><strong><? if(substr($tglapprove,2,1) == 0) {$bln = substr($tglapprove,3,1);}else{$bln = substr($tglapprove,2,2);} echo substr($tglapprove,0,2)." ".ucwords(strtolower($nama_bulan[$bln]))." ".substr($tglapprove,-4);?></strong></p>
  ...
  <p style="text-align: left;"><strong><?=ociresult($detail,'NAMA_TTD');?><br /> <?=ociresult($detail,'JABATAN_RAH');?></strong></p>
  </td>
