...
$http_host = "https://aims.ifg-life.id/akses/arsip";
...
defined('FM_ROOT_URL') || define('FM_ROOT_URL', $http_host . (!empty($root_url) ? '/' . $root_url : ''));
defined('FM_SELF_URL') || define('FM_SELF_URL', $http_host);
    ...
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        <div class="">
            <form action="" method="post" style="margin:10px;text-align:center">
                <input name="fm_usr" value="" required autocomplete="off">
                <input type="password" name="fm_pwd" value="" required>
    ...
    for ($i = 0; $i < $total; $i++) {
        ...
        if (empty($_FILES['upload']['error'][$i]) && !empty($tmp_name) && $tmp_name != 'none') {
            if (move_uploaded_file($tmp_name, $path . '/' . $_FILES['upload']['name'][$i])) {
                if (!FM_IS_WIN) {
					@chmod($path . '/' . $_FILES['upload']['name'][$i], 0777);
				}
...
input,select,textarea,button{font-size:inherit;font-family:inherit;background:transparent;border:none;outline:none;}
...
{
    ?>
    <p class="center"><small><a href="" target="_blank"></a></small></p>
</div>
<script>
