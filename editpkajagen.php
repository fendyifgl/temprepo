...
<?
	if ($submit=="SUBMIT") {  
		 ...
		 VALUES ('CREATED DOCUMENT eKAJ','$noidagen','$noidagen',$nomerx,'PDF',localtimestamp,'TESTINGxx','1')";
	...
	"(SELECT ALAMAT01||', '||ALAMAT02||', '||PROPINSI||'-'||KODEPOS FROM $DBUser.tabel_001_kantor WHERE kdkantor = '$kantor') AS alamatktr,".
      ...
      <table border="0" width="100%" cellspacing="2">
        ...
        <tr>
         ...
         <!--  <td bgcolor="#D8E2FC"><input type="text" name="alamatktr" size="40" value="<?=$arc["KDJABATANAGEN"]=="06"? "JL. Ir. H. Juanda No. 34 Jakarta - 10120": $ass["ALAMATKTR"];?>"></td> -->
          <td bgcolor="#D8E2FC"><input type="text" name="alamatktr" size="40" value="Graha CIMB Niaga Lt. 21, Jl. Jend. Sudirman Kav 58 Jakarta - 12190" readonly></td>
        ...
        <tr>
         ...
         <!--  <td bgcolor="#D8E2FC"><input type="text" name="phonektr" size="30" value="<?=$arc["KDJABATANAGEN"]=="06"? "021-3845031": $ass["PHONEKTR"];?>"></td> -->
          <td bgcolor="#D8E2FC"><input type="text" name="phonektr" size="30" value="021-3845031" readonly></td>
...
<table border="1" cellpadding="2" cellspacing="1" style="border-collapse: collapse" bordercolor="#CCCCCC" width="650" id="AutoNumber1">
  ...
  ?>
	...
	<?
  	$sql = " SELECT * FROM $DBUser.HISTORY_LOG_PKAJ WHERE DOCUMENT_NO = '$nomerx'";
	$listHistory=ociparse($conn, $sql);

	// DOCUMENT CREATED
	$createdForHeader = "SELECT *
	FROM ( select a.*, max(DATETIME) over () as max_created
			 from $DBUser.HISTORY_LOG_PKAJ a WHERE DOCUMENT_NO = '$nomerx' AND LOWER(ACTIVITY) LIKE '%created%'
				  )
	...
	$viewed = "SELECT *
	FROM ( select a.*, max(DATETIME) over () as max_created
			 from $DBUser.HISTORY_LOG_PKAJ a WHERE DOCUMENT_NO = '$nomerx' AND LOWER(ACTIVITY) LIKE '%viewed%'
				  )
   	WHERE DATETIME = max_created";
	...
	$otp = "SELECT *
	FROM ( select a.*, max(DATETIME) over () as max_created
			 from $DBUser.HISTORY_LOG_PKAJ a WHERE DOCUMENT_NO = '$nomerx' AND LOWER(ACTIVITY) LIKE '%otp%'
				  )
   	WHERE DATETIME = max_created";
	...
	$signed = "SELECT *
	FROM ( select a.*, max(DATETIME) over () as max_created
			 from $DBUser.HISTORY_LOG_PKAJ a WHERE DOCUMENT_NO = '$nomerx' AND LOWER(ACTIVITY) LIKE '%signed%'
	...
	$agreement = "SELECT *
	FROM ( select a.*, max(DATETIME) over () as max_created
			 from $DBUser.HISTORY_LOG_PKAJ a WHERE DOCUMENT_NO = '$nomerx' AND LOWER(ACTIVITY) LIKE '%agreement%'
				  )";
...
<table border="0" cellpadding="2" cellspacing="1" style="border-collapse: collapse" bordercolor="#CCCCCC" width="650" id="AutoNumber1">
	<tr>
		<td><label><span><b>"DOKUMEN PENANDATANGANAN EPKAJ <?= $nomerx ?>" - HISTORY</b></span></label></td>
	</tr> 
	<tr>
		<td><br></td>
	</tr>
	<tr bgcolor="#CBC8C7">
		...
		<td align="left">Created date : <?= oci_result($documentCreatedForHeader, 'DATETIME'); ?></td>
		<td></td>
	</tr>
	<tr bgcolor="#CBC8C7">
		<td align="left">by : <?= oci_result($documentCreatedForHeader, 'USR'); ?></td>
		<td></td>
	</tr>
	<tr bgcolor="#CBC8C7">
	...
	<tr bgcolor="#CBC8C7">
		<td align="left">Document no. : <?= $nomerx?></td>
		<td><a href="editpkajagen_export.php<?= $no?>" target="_blank" class="btn btn-success"><i class="fa fa-file-pdf-o"></i> Export to PDF</a></td>	
		<?php } ?>
	</tr>
	<tr>
		<td><br></td>
  	...
  	<tr>
  		<td bgcolor="white" align="left" >
			...
			<?php while (oci_fetch($documentCreated)) { ?>
				...
				<?= oci_result($documentCreated, 'ACTIVITY') . " by " . oci_result($documentCreated, 'USR'); ?> </br> <?= oci_result($documentCreated, 'DATETIME'); ?>
			<?php } ?>
			<!-- END DOCUMENT CREATED -->
			<br> <hr> <br>
			...
			<?php while (oci_fetch($documentViewed)) { 
				$docviewed = oci_result($documentViewed, 'ACTIVITY') . " by " . oci_result($documentViewed, 'USR');
			?>
				<i class="fa fa-eye"></i>
				<?= $docviewed; ?> </br> <?= oci_result($documentViewed, 'DATETIME'); ?>
			<?php } ?>
			<!-- END DOCUMENT CREATED -->
			<br> <hr> <br>
			...
			<?php while (oci_fetch($documentOTP)) { ?>
				...
				<?= oci_result($documentOTP, 'ACTIVITY'); ?> code sent to <?= oci_result($documentOTP, 'USR'); ?> via SMS </br> <?= oci_result($documentOTP, 'DATETIME'); ?>
			<?php } ?>
			<!-- END DOCUMENT OTP -->
			...
			?>
				...
				<?= $row["ACTIVITY"]; ?> by <?= $row["USR"]; ?>  </br> <?= $row["DATETIME"]; ?>
			<?php $i++; } ?>
			<!-- END DOCUMENT SIGNED -->
			<br> <hr> <br>
			...
			<?php while (oci_fetch($documentAgreement)) { ?>
				...
				<?= oci_result($documentAgreement, 'ACTIVITY'); ?> </br> <?= oci_result($documentAgreement, 'DATETIME'); ?>
			<?php } ?>
