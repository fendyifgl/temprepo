<?php
	...
	$user_approve = in_array($modul, array('ALL','POA'));
	// $user_approve = in_array($modul, array('ALL'));
?>
<html>
<head>
...
<body>
	<form action="<? $PHP_SELF; ?>" method="post" name="memreg">
		...
		if ($approve) {
			if (count($cekbox) == 0) {
				...
				foreach ($cekbox as $i => $v) {
					...
					$sql = "SELECT * FROM TABEL_100_KLIEN_REKENING_TMP WHERE {$where} AND STATUS = '0'";
					...
					if(count($res) > 0 && @$res){
						...
						$sql = "SELECT * FROM TABEL_100_KLIEN_REKENING WHERE {$where}";
						...
						if(count($rekOri) > 0 && @$rekOri){
							...
							// Update Rekening API
							$sql = "UPDATE $DBUser.tabel_100_klien_rekening_api SET REMARK = 'MANUAL', CODE = '0000', DESCRIPTION = 'Inquiry Success'
									WHERE {$where} AND ACCOUNTNUMBER = '{$res['NOREKENING']}'";
							$DB->parse($sql);
							$DB->execute();
							
							//echo "<script>alert('Success Update Data Rekening Di Setujui'); window.href.location = `./approval_update_rekening_manual.php` </script>";
						}else{
							echo "Tidak ada data yang disetujui";
						}
					}else{
		...
		if ($submit || $approve || $reject) { 
				...
				FROM TABEL_100_KLIEN_REKENING_TMP A 
					INNER JOIN TABEL_399_BANK B  ON A.KDBANK = B.KDBANK
					INNER JOIN NADM.TABEL_200_PERTANGGUNGAN C ON A.PREFIXPERTANGGUNGAN||A.NOPERTANGGUNGAN = C.PREFIXPERTANGGUNGAN||C.NOPERTANGGUNGAN
					INNER JOIN NADM.TABEL_100_KLIEN D ON C.NOPEMEGANGPOLIS = D.NOKLIEN
					INNER JOIN NADM.TABEL_999_KODE_STATUS E ON A.STATUS = E.KDSTATUS AND E.JENISSTATUS = 'REKENING'
					WHERE 1=1 
							$filtertanggal
							$filterstatus
